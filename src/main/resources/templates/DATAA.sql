...
INSERT INTO `tb_categoria` VALUES (1,'VIP'),(2,'Premiun'),(3,'Intermedio'),(4,'Básico');
...
INSERT INTO `tb_medicamento` VALUES (1,'Paracetamol 500 mg','Paracetamol',20,5.6,'2022-01-01',2),(2,'Lexotiroxina sódicax','Se encarga de sustituir una hormona que se suele producir en nuestra glándula tiroidea para regular la energía y el metabolismo del cuerpo. Es una versión artificial de la hormona tiroxina, responsable de aumentar la tasa metabólica de las células de todos los tejidos del organismo y ayuda a mantener la función cerebral, la absorción de los alimentos y la temperatura corporal, entre otros efectos. También la lista de la OMS.',25,25.6,'2025-06-24',2),(3,'Aspirina','También conocida como Ácido acetilsalicílico (ASA), reduce las sustancias en el cuerpo que producen dolor, fiebre e inflamación. Es el medicamento más usado en todo el mundo y se calcula que cada año se consumen unas 40.000 toneladas.',20,22,'2021-03-01',1),(4,'Omeprazol ','inhibe la bomba de protones (IBPS)y disminuye la producción de Ácido al bloquear la enzima de la pared del estomago que se encarga de producir esta sustancia. Este efecto reviene las ulceras y tiene un resultado curativo sobre las ulceras existentes en el esófago, estomago y duodeno. La OMS lo incluye como medicamento básico.',20,5,'2021-03-10',1),(5,'Lexotiroxina sódicas','Se encarga de sustituir una hormona que se suele producir en nuestra glándula tiroidea para regular la energía y el metabolismo del cuerpo. Es una versión artificial de la hormona tiroxina, responsable de aumentar la tasa metabólica de las células de todos los tejidos del organismo y ayuda a mantener la función cerebral, la absorción de los alimentos y la temperatura corporal, entre otros efectos. También la lista de la OMS.',25,25.6,'2025-06-24',2);
...
CREATE TABLE `tb_paciente` (
  ...
  `nom_pac` varchar(30) DEFAULT NULL,
  `ape_pac` varchar(30) DEFAULT NULL,
  `sexo_pac` varchar(15) DEFAULT NULL,
  `cod_cate_pac` int DEFAULT NULL,
  PRIMARY KEY (`cod_pac`),
  KEY `FK_0001` (`cod_cate_pac`),
  CONSTRAINT `FK_0001` FOREIGN KEY (`cod_cate_pac`) REFERENCES `tb_categoria` (`cod_cate`)
...
INSERT INTO `tb_paciente` VALUES (1,'dasda','bbb','Femenino',1);
...
-- Dump completed on 2023-09-18 22:10:15
